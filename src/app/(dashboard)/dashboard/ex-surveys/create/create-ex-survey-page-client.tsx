...
export function CreateEXSurveyPageClient() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!selectedTemplate) {
        setError("Selected template not found. Please go back and choose a template.");
        setSubmitting(false);
        return;
      }
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-8 pb-12">
      ...
      <div className="flex items-center gap-4">
        <Button variant="ghost" size="icon" asChild aria-label="Back to survey templates">
      ...
      <div className="min-h-[400px]">
        <AnimatePresence mode="wait" custom={direction}>
          <motion.div
            key={currentStep}
            custom={direction}
            // @ts-expect-error stepVariants uses custom direction functions not matching framer-motion Variants type
