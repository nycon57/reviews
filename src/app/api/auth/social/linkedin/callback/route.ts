...
export async function GET(request: NextRequest) {
  ...
  // Handle the callback
  const result = await handleSocialOAuthCallback('linkedin', code, state);
  ...
  // Check if page selection is needed (company pages)
  if (result.data?.needsPageSelection && result.data?.pages) {
    const pagesParam = encodeURIComponent(JSON.stringify(result.data.pages));
    return NextResponse.redirect(
      `${redirectBase}&select_page=linkedin&connection_id=${result.data.connectionId}&pages=${pagesParam}`
    );
  }

  // Redirect to settings with success message
  return NextResponse.redirect(
    `${redirectBase}&success=linkedin&connection_id=${result.data!.connectionId}`
  );
}
