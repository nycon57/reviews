...
import { refreshAccessToken, getReviews, isTokenExpired, STAR_RATING_MAP } from "@/lib/google";
...
export async function GET(request: NextRequest) {
  ...
  return withCronHeartbeat("google-sync", async () => {
    ...
    try {
      ...
      async function processConnection(connection: NonNullable<typeof connections>[0]) {
        ...
        try {
          ...
          do {
            ...
            const existingMap = new Map(existingReviews?.map((r) => [r.source_review_id, r]) || []);
            ...
            for (const googleReview of response.reviews) {
              ...
              if (existingReview) {
                ...
                console.warn(
                  `Skipping review ${googleReview.reviewId} - no loan officer assigned to connection ${connection.id}`
                );
              }
      ...
      const batchResults = await processBatch(connections, processConnection, CONCURRENCY_LIMIT);
      results.push(...batchResults);
