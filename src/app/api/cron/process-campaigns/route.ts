...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  return withCronHeartbeat("process-campaigns", async () => {
    try {
      ...
      const { data: campaigns, error: fetchError } = await supabase
        ...
        .select(
          "id, organization_id, name, description, status, sequence_definition, canvas_metadata, trigger_type"
        )
        .eq("status", "active");
      ...
      const { registerEmailSender, getSequenceDefinition } =
        await import("@/lib/email/orchestration/registry");
      const { processSequenceQueue } = await import("@/lib/email/orchestration/queue");
      ...
      for (const row of activeCampaigns) {
        ...
        const hasActiveSequences = sequences.some(
          (s) =>
            ...
            (s as { status: string }).status === "processing"
        );
      ...
      return NextResponse.json({
        ...
        autoCompleted,
        errors: [...triggerResults.flatMap((r) => r.errors), ...queueResult.errors].slice(0, 10),
