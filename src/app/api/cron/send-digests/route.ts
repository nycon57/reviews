...
export async function POST(request: NextRequest) {
  ...
  return withCronHeartbeat("send-digests", async () => {
    try {
      const supabase = createUntypedAdminClient();
      const resend = getResendClient();

      // Get users who need digest emails
      const { data: usersNeedingDigest, error: usersError } = (await supabase.rpc(
        "get_users_needing_digest"
      )) as { data: DigestUser[] | null; error: Error | null };
      ...
      for (const user of usersNeedingDigest) {
        try {
          ...
          const [profileResult, queueResult] = await Promise.all([
            supabase.from("users").select("id, email, full_name").eq("id", user.user_id).single(),
            supabase
          ...
          if (queueError) {
            errors.push(
              `Failed to get notifications for user ${user.user_id}: ${queueError.message}`
            );
            failed++;
            continue;
          }
