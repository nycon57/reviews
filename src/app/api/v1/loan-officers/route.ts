...
import { loanOfficerFiltersSchema, validateParams } from '@/lib/api/validation';
...
// GET /api/v1/loan-officers - List loan officers
async function handleGet(
  ...
  // Parse filters
  const filtersResult = validateParams(loanOfficerFiltersSchema, searchParams);
  ...
  if (error) {
    ...
    return apiInternalError(context.requestId, 'Failed to fetch loan officers');
  }

  const loanOfficers = (data || []).map((row) =>
    mapLoanOfficerRow(row as Record<string, unknown>)
  );

  return apiPaginated(loanOfficers, { page, pageSize, total: count || 0 }, context.requestId);
}
...
export const GET = withApiAuth(handleGet, ['loan-officers:read']);
