...
import { reviewResponseSchema, validateBody } from '@/lib/api/validation';
...
// POST /api/v1/reviews/:id/respond - Add response to a review
async function handlePost(
  ...
  const { data: existing, error: existError } = await supabase
    .from('reviews')
    .select('id, platform, platform_review_id')
    .eq('id', id)
  ...
  const { data: review, error: updateError } = await supabase
    .from('reviews')
    .update({
      response_text: validation.data.response_text,
  ...
  if (updateError) {
    ...
    return apiInternalError(context.requestId, 'Failed to add review response');
  }

  // Note: For Google reviews, you would need to call the Google Business Profile API
  // to actually post the response. This just stores it locally for now.
...
export const POST = (request: NextRequest, routeContext: RouteParams) =>
  withApiAuth(
    (req, ctx) => handlePost(req, ctx, routeContext),
    ['reviews:write']
  )(request);
