...
export function BranchUploadStep({ onUpload }: UploadStepProps) {
  const [error, setError] = useState<string | null>(null);
  const [fileName, setFileName] = useState<string | null>(null);

  const handleDownloadTemplate = useCallback(() => {
    const csv = generateBranchCSVTemplate();
    const blob = new Blob([csv], { type: "text/csv" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = "branch-import-template.csv";
    a.click();
    ...
    (acceptedFiles: File[]) => {
      ...
      Papa.parse<ParsedCSVRow>(file, {
        ...
        complete: (results) => {
          ...
          if (rows.length > MAX_BRANCH_IMPORT_ROWS) {
            setError(
              `Too many rows. Maximum is ${MAX_BRANCH_IMPORT_ROWS}, found ${rows.length}`
            );
            return;
          }
      ...
      >
        ...
        {fileName ? (
          ...
          <>
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Max {MAX_BRANCH_IMPORT_ROWS} rows, 1MB limit
            </p>
          </>
        )}
      </div>
