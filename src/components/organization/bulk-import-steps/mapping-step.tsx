...
import { REQUIRED_FIELDS } from "@/lib/organization/bulk-import-types";
import {
...
interface MappingStepProps {
  ...
  onMappingChange: (index: number, fieldKey: CSVFieldKey | null) => void;
}
...
export function MappingStep({
  ...
  const missingRequired = getMissingRequiredFields(mappings);
        ...
        <Table>
          ...
          <TableBody>
            {headers.map((header, idx) => {
              ...
              const options = getAvailableFieldOptions(
              ...
              return (
                <TableRow key={header}>
                  <TableCell className="font-medium">
                    {header}
                    {mapping?.fieldKey &&
                      REQUIRED_FIELDS.includes(mapping.fieldKey) && (
                  ...
                  <TableCell>
                    <Select
                      ...
                      onValueChange={(val) =>
                        onMappingChange(
                          idx,
                          val === SKIP_VALUE ? null : (val as CSVFieldKey)
                        )
                      }
                    >
