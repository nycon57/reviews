  ...
  return (
    <div className="flex h-full flex-col">
      <div className="border-b border-border px-3 py-2">
        <h3 className="text-xs font-semibold text-foreground">Layers ({state.elements.length})</h3>
      </div>
            ...
            sortedElements.map((el, index) => (
              ...
              >
                ...
                <span className="min-w-0 flex-1 truncate text-[11px] text-foreground">
                  {el.text?.slice(0, 20) || el.statLabel || el.shape || el.type}
                </span>
...
export function ShareStudioLayerPanel({ editor }: LayerPanelProps) {
  return <LayerPanel editor={editor} />;
}

export function SocialGraphicsLayerPanel({ editor }: LayerPanelProps) {
  return <LayerPanel editor={editor} />;
}
