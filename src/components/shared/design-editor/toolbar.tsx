...
import { Undo2, Redo2, ZoomIn, ZoomOut, Maximize, Save, Grid3X3, Palette } from "lucide-react";
import type { CanvasBackground, CanvasSize, DesignEditorConfig, EditorActions } from "./types";
...
interface NamedCanvasPreset extends CanvasSize {
  name: string;
}

function hasPresetName(preset: CanvasSize): preset is NamedCanvasPreset {
  return Boolean(preset.name);
}

function getCanvasPresets(config: DesignEditorConfig): NamedCanvasPreset[] {
  if (config.filterUnnamedCanvasPresets) {
    return config.canvasPresets.filter(hasPresetName);
  }

  return config.canvasPresets.map((preset) => ({
    ...preset,
    name: preset.name ?? "",
  }));
}

function useCustomSizeState(
  external: number,
  shouldSync: boolean
): [number, Dispatch<SetStateAction<number>>] {
  const synced = useSyncedState(external);
  const local = useState(external);
  return shouldSync ? synced : local;
}

export function EditorToolbar({
  editor,
  config = SHARE_STUDIO_DESIGN_EDITOR_CONFIG,
  graphicName,
  onNameChange,
  onSave,
  isSaving,
  ...
  const [customWidth, setCustomWidth] = useCustomSizeState(
    state.canvasSize.width,
    config.syncCustomSizeInputs
  );
  const [customHeight, setCustomHeight] = useCustomSizeState(
    state.canvasSize.height,
    config.syncCustomSizeInputs
  );
  const canvasPresets = getCanvasPresets(config);
  ...
  const handlePresetChange = useCallback(
    (value: string) => {
      ...
      const preset = config.canvasPresets.find((p) => p.name === value);
      if (preset) setCanvasSize(preset);
    },
    [config.canvasPresets, setCanvasSize]
  );
  ...
  return (
    <div className="flex items-center gap-2 border-b border-border bg-background-subtle px-4 py-2">
      ...
      {/* Canvas Size */}
      <Select value={state.canvasSize.name ?? "Custom"} onValueChange={handlePresetChange}>
        ...
        <PopoverContent className="w-64 space-y-3 p-3" align="start">
            ...
            <div>
              ...
              <div className="flex items-center gap-1">
                <input
                  ...
                  onChange={(e) => setBackground({ ...bg, color: e.target.value })}
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setBackground({ ...bg, gradientFrom: e.target.value })}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setBackground({ ...bg, gradientTo: e.target.value })}
                    className="h-7 w-full cursor-pointer rounded border border-border"
                  />
                </div>
              </div>
          ...
          {bg.type === "image" && (
            <div>
              ...
              <Input
                value={bg.imageUrl ?? ""}
                onChange={(e) => setBackground({ ...bg, imageUrl: e.target.value })}
                placeholder="https://..."
                className="h-7 text-xs"
              />
            </div>
      ...
      {/* Save */}
      {saved && <span className="text-xs text-repwell-sage-200">Saved</span>}
      <Button size="sm" className="h-8 gap-1" onClick={onSave} disabled={isSaving}>
...
export function ShareStudioEditorToolbar({
  editor,
  graphicName,
  onNameChange,
  onSave,
...
export function SocialGraphicsEditorToolbar({
  editor,
  graphicName,
  onNameChange,
  onSave,
  ...
  return (
    <EditorToolbar
      editor={editor}
      config={SOCIAL_GRAPHICS_DESIGN_EDITOR_CONFIG}
      graphicName={graphicName}
      onNameChange={onNameChange}
      onSave={onSave}
      isSaving={isSaving}
