...
export const WidgetDetailAnalyticsPanel = memo(function WidgetDetailAnalyticsPanel({
  widgetId,
  widgetName,
  dateRange,
  customStart,
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/dashboard/widgets/${widgetId}/analytics?${params}`);
      if (!res.ok) {
  ...
  const handleEventExport = useCallback(() => {
    startExport(async () => {
      const result = await getWidgetEventLevelCsvData(widgetId, dateRange, customStart, customEnd);
      if (!result.success) {
  ...
  const pieData = useMemo(
    () =>
      (data?.eventBreakdown ?? []).map((e) => ({
        ...
        value: e.count,
        color: EVENT_COLORS[e.eventType] ?? "hsl(var(--chart-2))",
      })),
    [data?.eventBreakdown]
  );
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-semibold text-heading">{widgetName ?? widgetId}</h2>
              ...
              <>
                <div className="h-[250px] w-full" aria-hidden="true">
                  <ResponsiveContainer width="100%" height="100%">
                    ...
                    >
                      <defs>
                        <linearGradient id="detail-grad-imp" x1="0" y1="0" x2="0" y2="1">
                          <stop
                            offset="5%"
                            stopColor={DETAIL_CHART_COLORS.impressions}
                            stopOpacity={0.3}
                          />
                          <stop
                            offset="95%"
                        ...
                        <linearGradient id="detail-grad-clk" x1="0" y1="0" x2="0" y2="1">
                          <stop
                            offset="5%"
                            stopColor={DETAIL_CHART_COLORS.clicks}
                            stopOpacity={0.3}
                          />
                          <stop
                            offset="95%"
                      ...
                      <Area
                        ...
                        dataKey="impressions"
                        name="Impressions"
                        stroke={DETAIL_CHART_COLORS.impressions}
                        strokeWidth={2}
                      ...
                      <Area
                        ...
                        dataKey="clicks"
                        name="Clicks"
                        stroke={DETAIL_CHART_COLORS.clicks}
                        strokeWidth={2}
      ...
      <div className="grid gap-6 lg:grid-cols-2">
                ...
                {data.topPageUrls.map((item) => (
                  ...
                  >
                    <a
                      ...
                      className="flex max-w-[80%] items-center gap-1 truncate text-xs text-repwell-teal-300 hover:text-repwell-teal-400 dark:hover:text-muted-foreground"
                    >
                      {item.url}
        ...
        <Card className="border-border">
          <CardHeader>
            ...
            <CardDescription>Where your widget traffic comes from</CardDescription>
          </CardHeader>
          <CardContent>
              ...
              <div className="space-y-2">
                {data.topReferrers.map((item) => (
                  <div
                    ...
                    className="flex items-center justify-between gap-3 border-b border-border-subtle py-2 last:border-0"
                  >
                    <span className="max-w-[80%] truncate text-xs text-label">{item.referrer}</span>
                    <span className="text-xs font-medium tabular-nums text-muted-foreground">
                      {item.count.toLocaleString()}
                    </span>
                  </div>
        ...
        <CardHeader>
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <CardDescription>Unique visitor distribution</CardDescription>
            </div>
          </div>
        </CardHeader>
        <CardContent>
          {!data?.geographicBreakdown?.length ? (
            ...
            <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
              {data.geographicBreakdown.map((item) => (
                ...
                >
                  <span
                    className="max-w-[70%] truncate font-mono text-xs text-muted-foreground"
                    title={item.ipHash}
                  >
      ...
      <CardHeader>
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <CardDescription>Percentage of visitors reaching each depth threshold</CardDescription>
          </div>
        </div>
      </CardHeader>
      <CardContent>
          ...
          {data.map((item) => (
            <div key={item.threshold} className="space-y-1">
              <div className="flex items-center justify-between text-xs">
                <span className="font-medium text-foreground">{item.threshold}%</span>
      ...
      <CardHeader>
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <CardDescription>Play rate, completion rate, and watch duration</CardDescription>
          </div>
        </div>
      </CardHeader>
      <CardContent>
        <div className="mb-6 grid gap-4 sm:grid-cols-3">
          <div className="rounded-lg border border-border p-4 text-center">
            ...
            <div className="mt-0.5 text-xs text-muted-foreground">Play Rate</div>
          ...
          <div className="rounded-lg border border-border p-4 text-center">
            ...
            <div className="mt-0.5 text-xs text-muted-foreground">Completion Rate</div>
            <div className="text-[10px] text-muted-foreground">
          ...
          <div className="rounded-lg border border-border p-4 text-center">
            ...
            <div className="mt-0.5 text-xs text-muted-foreground">Avg Watch Time</div>
          </div>
        </div>

        {/* Video milestones */}
        <div>
          <h4 className="mb-2 text-xs font-medium text-muted-foreground">Viewer Milestones</h4>
          <div className="space-y-2">
            {data.milestones.map((m) => {
              const pct = data.totalPlays > 0 ? Math.round((m.count / data.totalPlays) * 100) : 0;
              return (
                <div key={m.milestone} className="space-y-1">
                  <div className="flex items-center justify-between text-xs">
                    <span className="font-medium text-foreground">{m.milestone}% watched</span>
                    <span className="tabular-nums text-muted-foreground">
...
function ConversionFunnelCard({ data }: { data: EnhancedAnalytics["conversions"] }) {
  const steps = [
    {
      label: "Impressions",
      ...
      <CardContent>
          ...
          {steps.map((step, i) => {
            ...
            return (
              <div key={step.label} className="flex flex-1 items-center gap-2 sm:gap-4">
                <div className="flex-1 rounded-lg border border-border p-3 text-center sm:p-4">
                  <Icon size={14} className="mx-auto mb-1.5 text-repwell-teal-300" />
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-3">
          <div className="rounded-lg bg-muted/50 p-3 text-center">
            ...
            <div className="text-[10px] text-muted-foreground">Impression → Click</div>
          </div>
          <div className="rounded-lg bg-muted/50 p-3 text-center">
            <div className="text-sm font-semibold tabular-nums text-foreground">
              {data.clickToConversionRate}%
            </div>
            <div className="text-[10px] text-muted-foreground">Click → Conversion</div>
          </div>
          <div className="rounded-lg bg-muted/50 p-3 text-center">
            <div className="text-sm font-semibold tabular-nums text-foreground">
