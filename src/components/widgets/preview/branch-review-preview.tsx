...
function BranchHeader({ profile, starFilled, starEmpty }: {
      ...
      <div className="flex-1 min-w-0">
        ...
        {profile.average_rating != null && (
          <div className="flex items-center gap-2 mt-1">
            ...
            <StarRating rating={profile.average_rating} filledColor={starFilled} emptyColor={starEmpty} />
...
function SourceBreakdownSection({ sources, starFilled, starEmpty }: {
  ...
  return (
    <div className="mb-4 p-4 rounded-lg" style={{ border: "1px solid var(--rw-border, #e5e7eb)" }}>
      <h4 className="text-sm font-semibold mb-3" style={{ color: "var(--rw-text, #1a1a2e)" }}>Reviews by Source</h4>
          ...
          return (
            <div key={src.source} className="flex items-center gap-2.5">
              ...
              <div className="flex-1 min-w-0">
                ...
                <span className="text-[11px] text-gray-400">{src.count} reviews &middot; {src.average.toFixed(1)} avg</span>
              </div>
              <StarRating rating={src.average} filledColor={starFilled} emptyColor={starEmpty} size={12} />
            </div>
          );
        })}
      </div>
...
function TeamMemberGrid({ members, starFilled, starEmpty }: {
  members: TeamMember[];
  starFilled: string;
  starEmpty: string;
}) {
  return (
    <div className="mb-4 p-4 rounded-lg" style={{ border: "1px solid var(--rw-border, #e5e7eb)" }}>
      <h4 className="text-sm font-semibold mb-3" style={{ color: "var(--rw-text, #1a1a2e)" }}>Our Team</h4>
      <div className="grid grid-cols-3 gap-3 max-[768px]:grid-cols-2 max-[480px]:grid-cols-1">
        {members.map((member) => (
          <div key={member.id} className="flex flex-col items-center text-center p-3 rounded-lg bg-gray-50 transition-shadow hover:shadow-sm">
            {member.photo_url ? (
              <img src={member.photo_url} alt={member.full_name ?? "Team member"} className="w-12 h-12 rounded-full object-cover mb-2 bg-gray-200" />
            ) : (
            ...
            {member.full_name && (
              <div className="text-[13px] font-semibold truncate w-full" style={{ color: "var(--rw-text, #1a1a2e)" }}>
                {member.full_name}
              </div>
            )}
            {member.title && <div className="text-[11px] text-gray-400 truncate w-full">{member.title}</div>}
            {member.average_rating != null && (
              <div className="flex items-center gap-1 mt-1">
                ...
                <StarRating rating={member.average_rating} filledColor={starFilled} emptyColor={starEmpty} size={10} />
    ...
    <article className={cardClasses} tabIndex={0}>
      <div className="flex items-center gap-2.5 mb-2">
        {content.showAvatar !== false && (
          review.avatar_url ? (
            <img src={review.avatar_url} alt={review.reviewer_name ?? "Reviewer"} className="w-9 h-9 rounded-full object-cover flex-shrink-0 bg-gray-200" />
          ) : (
...
export function BranchReviewPreview({
  ...
  return (
    <div
      ...
      role="region"
      aria-label={content.headerText ?? `Reviews for ${profile?.organization_name ?? "Branch"}`}
    >
      {/* Branch Header */}
      {profile && content.showHeader !== false && (
        <BranchHeader profile={profile} starFilled={starFilled} starEmpty={starEmpty} />
      )}

      {/* Rating Distribution */}
      {content.showRatingDistribution !== false && profile?.rating_distribution && profile.total_reviews ? (
        <RatingDistributionChart distribution={profile.rating_distribution} totalReviews={profile.total_reviews} starFilled={starFilled} />
      ) : null}

      {/* Source Breakdown */}
      {content.showSourceBreakdown !== false && profile?.source_breakdown && profile.source_breakdown.length > 0 && (
        <SourceBreakdownSection sources={profile.source_breakdown} starFilled={starFilled} starEmpty={starEmpty} />
      )}

      {/* Team Members */}
      {content.showTeam !== false && profile?.team_members && profile.team_members.length > 0 && (
        <TeamMemberGrid members={profile.team_members} starFilled={starFilled} starEmpty={starEmpty} />
      )}
      ...
      {reviews.length === 0 ? (
        <div className="py-8 text-center">
          <div className="text-gray-400 text-sm mb-3">No reviews yet for this branch.</div>
        ...
        <div className="grid gap-3" style={{ gridTemplateColumns: columns > 1 ? `repeat(${columns}, 1fr)` : "1fr" }}>
          {displayReviews.map((review) => (
            <ReviewCard key={review.id} review={review} content={content} starFilled={starFilled} starEmpty={starEmpty} />
          ))}
        </div>
      )}
