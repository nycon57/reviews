...
export async function handleGoogleOAuthCallback(
  ...
  try {
    ...
    if (professionalId) {
      ...
      if (!professional) {
        return { success: false, error: "Selected professional is not in your organization" };
      }
    }
...
export async function syncGoogleReviews(
  ...
  try {
    ...
    const averageRating =
      allReviews.length > 0
        ? allReviews.reduce((sum, r) => sum + STAR_RATING_MAP[r.starRating], 0) / allReviews.length
        : 0;
...
async function markGoogleReplyOutcome(
  ...
  const { error } = await client.from("google_review_replies").update(update).eq("id", replyId);
  if (error) {
...
export async function processPendingGoogleReplies(
  ...
  try {
    ...
    return { processed: 0, posted: 0, failed: 0, errors: ["Unauthorized"] };
  }
