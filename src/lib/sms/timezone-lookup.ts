/**
 * US area code to IANA timezone mapping.
 *
 * Used by the quiet hours engine to resolve the recipient's local time
 * from their phone number's area code. When a phone number spans multiple
 * timezones, the most-populated zone for that area code is used.
 *
 * Source: NANPA area code assignments + US Census population data.
 */
...
/**
 * Resolve the IANA timezone for a US phone number based on its area code.
 * Returns null if the area code is not recognized.
 */
export function getTimezoneForPhone(e164Phone: string): string | null {
  const match = e164Phone.match(/^\+1(\d{3})/);
  if (!match) return null;
  return AREA_CODE_TIMEZONE[match[1]] ?? null;
}

/**
 * Default TCPA quiet hours: 8:00 AM to 9:00 PM local time.
 */
export const TCPA_DEFAULT_QUIET_START = "21:00";
export const TCPA_DEFAULT_QUIET_END = "08:00";
