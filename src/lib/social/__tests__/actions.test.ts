...
const {
  ...
  createTwitterPkcePairMock: vi.fn(),
...
vi.mock("../client", () => ({
  createTwitterPkcePair: createTwitterPkcePairMock,
...
describe("social OAuth state hardening", () => {
  beforeEach(() => {
    ...
    createTwitterPkcePairMock.mockReturnValue({
      codeVerifier: "a".repeat(64),
      codeChallenge: "twitter-code-challenge",
    });
  ...
  it("rejects platform-mismatched signed state before exchanging tokens", async () => {
    ...
    const result = await handleSocialOAuthCallback("twitter", "oauth-code", state!);
